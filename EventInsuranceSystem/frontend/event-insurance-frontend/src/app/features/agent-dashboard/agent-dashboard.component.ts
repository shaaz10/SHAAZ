...
export class AgentDashboardComponent implements OnInit, AfterViewChecked {
  ...
  loadRequests() {
    this.agentSvc.getMyRequests().subscribe({
      next: r => {
        ...
        this.requests = activeRequests.map((x: any) => ({ ...x, _selectedProduct: null, _suggestedPremium: null, _msg: '', _err: false, _suggestions: [] }));
  ...
  loadApplications() {
    this.agentSvc.getMyApplications().subscribe({
      next: r => {
        ...
        this.applications = activeApps.map((a: any) => ({ ...a, _notes: '', _msg: '', _err: false }));
        this.cdr.detectChanges();
      },
      error: () => { }
    });
  ...
  loadCommissions() {
    ...
    this.agentSvc.getMyCommissions(id).subscribe({
      next: r => { this.commissions = r; this.cdr.detectChanges(); },
      error: () => { }
    });
  }
  ...
  private renderAgentChart() {
    import('chart.js').then(m => {
      ...
      switch (this.agentSelectedChart) {
        case 'commByPolicy': {
          const grouped: any = {};
          this.commissions.forEach((c: any) => { const k = 'Policy #' + c.policyId; grouped[k] = (grouped[k] || 0) + (c.amount || 0); });
          ...
          config = { type: 'bar', data: { labels, datasets: [{ label: 'Commission (\u20b9)', data, backgroundColor: [C.green, C.navy, C.gold, C.red], borderRadius: 8, barPercentage: 0.6 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { callback: (v: any) => '\u20b9' + v.toLocaleString() } } } } };
          break;
        }
        case 'workBreakdown': {
