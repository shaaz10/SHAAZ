...
export class AdminDashboardComponent implements OnInit, AfterViewChecked {
  ...
  loadAllData() {
    ...
    this.adminSvc.getAllPayments().subscribe({
      next: r => { this.payments = r; this.cdr.detectChanges(); },
  ...
  loadApprovals() {
    this.adminSvc.getEscalatedApplications().subscribe({
      next: r => { this.approvals = r.map((a: any) => ({ ...a, notes: '' })); this.cdr.detectChanges(); },
  ...
  loadPendingPolicies() {
    this.adminSvc.getApprovedApplications().subscribe({
      next: r => { this.pendingPolicies = r; this.cdr.detectChanges(); },
      error: () => { console.warn('Failed to load approved applications'); }
    });
  }

  approveApplication(app: any) {
    this.adminSvc.approveApplication(app.id, { approvalNotes: app.notes || 'Approved.' }).subscribe({
      next: () => {
        app.status = 'Approved';
  ...
  loadClaims() {
    this.claimSvc.getAll().subscribe({
      next: r => {
        ...
        this.unassignedClaims = r.filter((c: any) =>
          c.status === 'Submitted' ||
          c.status === 'Pending' ||
